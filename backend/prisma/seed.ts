...
async function main() {
  ...
  const extraProjectRows = [
    ['HX-BOLE-VILLA', 'Bole Residential Villa', 'Bole district residential villa project', 'Bole, Addis Ababa', '8500000.0000'],
    ['HX-SUMMIT-WH', 'Summit Warehouse Extension', 'Industrial warehouse extension and roofing works', 'Summit Industrial Area', '12000000.0000'],
    ['HX-AYAT-MIXED', 'Ayat Mixed-Use Block', 'Mixed-use block mobilization and excavation', 'Ayat, Addis Ababa', '24000000.0000'],
  ] as const;

  for (const [code, name, description, location, budget] of extraProjectRows) {
    const extraProject = await prisma.project.upsert({
      where: { code },
      update: { name, description, location, status: ProjectStatus.ACTIVE, budget, createdById: manager.id },
      create: { code, name, description, location, status: ProjectStatus.ACTIVE, budget, createdById: manager.id },
    });

    for (const [index, phaseName] of ['Mobilization', 'Foundation', 'Structure', 'Electrical', 'Finishing'].entries()) {
      await prisma.projectPhase.upsert({
        where: { projectId_name: { projectId: extraProject.id, name: phaseName } },
        update: { sequence: index + 1, budget: String((index + 1) * 300000) },
        create: { projectId: extraProject.id, name: phaseName, sequence: index + 1, budget: String((index + 1) * 300000) },
      });
    }

    const extraSite = await prisma.site.upsert({
      where: { code: `${code}-SITE` },
      update: { name: `${name} Site`, projectId: extraProject.id, createdById: siteEngineer.id, qrCodeValue: `homex://site/${code}-SITE` },
      create: { code: `${code}-SITE`, name: `${name} Site`, projectId: extraProject.id, createdById: siteEngineer.id, qrCodeValue: `homex://site/${code}-SITE` },
    });

    await prisma.warehouse.upsert({
      where: { code: `${code}-STORE` },
      update: { name: `${name} Site Store`, type: WarehouseType.PROJECT_SITE, projectId: extraProject.id, siteId: extraSite.id, createdById: storekeeper.id, qrCodeValue: `homex://warehouse/${code}-STORE` },
      create: { code: `${code}-STORE`, name: `${name} Site Store`, type: WarehouseType.PROJECT_SITE, projectId: extraProject.id, siteId: extraSite.id, createdById: storekeeper.id, qrCodeValue: `homex://warehouse/${code}-STORE` },
    });
  }
